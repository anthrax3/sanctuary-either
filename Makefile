...
XYZ = node_modules/.bin/xyz --repo git@github.com:sanctuary-js/sanctuary-either.git --script scripts/prepublish
...
README.md.tmp: index.js
	$(TRANSCRIBE) \
	  --heading-level 4 \
	  --url 'https://github.com/sanctuary-js/sanctuary-either/blob/v$(VERSION)/{filename}#L{line}' \
	  -- $^ \
